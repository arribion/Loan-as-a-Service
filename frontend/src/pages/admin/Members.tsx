...
export default function Overview(): JSX.Element {
  ...
  // Demo helpers (used by quick actions)
  const addDemoMember = () => {
    if (totalMembers >= memberCap) {
      push(`Member limit reached on ${planById(user!.plan).name}.`, "warn");
      return;
    }
    const m: Member = {
      id: `M-00${49 + added.length}`,
      name: `New Member ${added.length + 1}`,
      phone: "07XXXXXXXX",
      joined: "Today",
      savings: 0,
      activeLoans: 0,
    ...
    setAdded((s) => [m, ...s]);
    push(`${m.name} added — welcome member #${totalMembers + 1}.`);
  };
            ...
            <div className="rounded-2xl border border-ink/8 bg-cream p-5 shadow-card">
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="rounded-md border border-ink/8 px-3 py-2 text-sm font-semibold inline-flex items-center gap-2">
                  <PlusIcon /> Add demo member
                </button>
